...
export default function CcrPage() {
        ...
        Papa.parse(csvText, {
          ...
          complete: (results) => {
            const parsed = results.data.map(row => ({
              ...
              isClean:
                row.Is_Clean_Claim === true ||
                row.Is_Clean_Claim === 1 ||
                String(row.Is_Clean_Claim).toLowerCase() === "true" ||
                String(row.Is_Clean_Claim).trim() === "1",
  ...
  const kpiMetrics = useMemo(() => {
    const getMetricsForMonth = (month) => {
      ...
      if (!filtered.length) {
        return { totalClaims: 0, cleanClaims: 0, ccr: 0 };
      }
      const totalClaims = filtered.length;
      const cleanClaims = filtered.filter(d => d.isClean).length;
      const ccr = totalClaims > 0 ? (cleanClaims / totalClaims) * 100 : 0;
      return { totalClaims, cleanClaims, ccr };
    };
    ...
    return {
      overallCcr: mainMetrics.ccr.toFixed(2),
      totalClaims: mainMetrics.totalClaims,
      cleanClaims: mainMetrics.cleanClaims,
      ccrTrend: getTrendDetails(currentMetrics.ccr, previousMetrics?.ccr || 0),
      claimsTrend: getTrendDetails(currentMetrics.totalClaims, previousMetrics?.totalClaims || 0),
      cleanClaimsTrend: getTrendDetails(currentMetrics.cleanClaims, previousMetrics?.cleanClaims || 0),
    };
  }, [data, selectedClient, selectedMonth]);

  const filteredMonthlyData = useMemo(() => {
    const INDUSTRY_STANDARD_CCR_TARGET = 98;
    const monthlyAggregated = {};

    data.forEach((d) => {
      if (selectedClient === "All" || d.client === selectedClient) {
        const month = d.month;
        if (!monthlyAggregated[month]) monthlyAggregated[month] = { total: 0, clean: 0 };
        monthlyAggregated[month].total += 1;
        if (d.isClean) monthlyAggregated[month].clean += 1;
      }
    });
    ...
    return monthsToShow.map((month) => {
      const vals = monthlyAggregated[month] || { total: 0, clean: 0 };
      const actual = vals.total > 0 ? (vals.clean / vals.total) * 100 : 0;
      return {
        month,
        actual: +actual.toFixed(2),
        target: INDUSTRY_STANDARD_CCR_TARGET,
      };
    });
  ...
  const avgVsLastMonthData = useMemo(() => {
    ...
    const getCcrForMonth = (m) => {
      const monthData = clientFiltered.filter(d => d.month === m);
      if (!monthData.length) return 0;
      const clean = monthData.filter(d => d.isClean).length;
      return (clean / monthData.length) * 100;
    };

    const avg3 = last3.reduce((sum, m) => sum + getCcrForMonth(m), 0) / (last3.length || 1);
    const lastMonthCcr = getCcrForMonth(lastMonth);

    return [
      { label: "Last 3 Months Avg", ccr: +avg3.toFixed(2) },
      { label: `Last Month (${lastMonth})`, ccr: +lastMonthCcr.toFixed(2) },
    ];
  }, [data, selectedClient]);
  ...
  const aggregatedPayerData = useMemo(() => {
    ...
    filtered.forEach((item) => {
      if (!map[item.payer]) {
        map[item.payer] = { name: item.payer, cleanClaims: 0 };
      }
      if (item.isClean) map[item.payer].cleanClaims += 1;
    });

    // Filter out payers with 0 cleanClaims before returning
    return Object.values(map).filter((payerData) => payerData.cleanClaims > 0);
  }, [data, selectedClient, selectedMonth]);
      ...
      <div className="summary-filters">
        <div className="summary-boxes">
          <div className="summary-box" style={{ position: "relative" }}>
            <h4>Overall CCR</h4>
            <p>{kpiMetrics.overallCcr}%</p>
            <TrendIndicator trendInfo={kpiMetrics.ccrTrend} />
          </div>
          <div className="summary-box" style={{ position: "relative" }}>
            <h4>Total Claims</h4>
            <p>{kpiMetrics.totalClaims.toLocaleString()}</p>
            <TrendIndicator trendInfo={kpiMetrics.claimsTrend} />
          </div>
          <div className="summary-box" style={{ position: "relative" }}>
            <h4>Clean Claims</h4>
            <p>{kpiMetrics.cleanClaims.toLocaleString()}</p>
            <TrendIndicator trendInfo={kpiMetrics.cleanClaimsTrend} />
          </div>
        </div>
        ...
        <div className="chart-card">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={filteredMonthlyData} margin={{ top: 5, right: 10, bottom: 0, left: -15 }}>
              ...
              <Tooltip />
              <Legend />
              <Line type="monotone" dataKey="actual" stroke="#10b981" name="Actual CCR" />
              <Line type="monotone" dataKey="target" stroke="#ef4444" name="Target CCR" />
            </LineChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="chart-card">
          ...
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={avgVsLastMonthData}>
              ...
              <YAxis tickFormatter={(t) => `${t}%`} domain={[0, 100]} />
              <Tooltip />
              <Bar dataKey="ccr" fill="#10b981" barSize={40} name="CCR" />
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="chart-card" style={{ position: "relative" }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            ...
            >
              ...
              <Legend />
              <Bar dataKey="cleanClaims" fill="#10b981" barSize={7} name="Clean Claims" />
            </BarChart>
          </ResponsiveContainer>
      ...
      <div className="claim-table">
        ...
        <div className="claim-table-body">
          <table>
            <thead>
              <tr>
                {["Claim ID", "Billed Amount ($)", "Paid Amount ($)", "Month", "Payer"].map(
                  (header) => (
