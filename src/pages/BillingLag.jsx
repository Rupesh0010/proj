...
export default function BillingLagPage() {
        ...
        Papa.parse(csvText, {
          ...
          complete: results => {
            const parsed = results.data
              .filter(row => row.Charge_Entry_Date && row.Claim_Submission_Date) // Ensure dates exist
              .map((row) => ({
                id: row.Claim_ID,
                ...
                billingLag: dayjs(row.Claim_Submission_Date).diff(dayjs(row.Charge_Entry_Date), 'day'),
              }));
            setData(parsed);
          }
        });
  ...
  const kpiMetrics = useMemo(() => {
    const getMetrics = (dataset) => {
      ...
      const totalLag = dataset.reduce((sum, d) => sum + d.billingLag, 0);
    ...
    return {
      avgBillingLag: mainMetrics.avgLag.toFixed(1),
  ...
  const filteredMonthlyData = useMemo(() => {
    ...
    return monthsToShow.map(month => {
      ...
      const avgLag = monthRows.reduce((sum, r) => sum + r.billingLag, 0) / monthRows.length;
  ...
  const avgVsLastMonthData = useMemo(() => {
    const getAvgLagForMonth = (monthStr) => {
      ...
      return monthData.reduce((sum, d) => sum + d.billingLag, 0) / monthData.length;
    };
  ...
  const aggregatedPayerData = useMemo(() => {
    ...
    filtered.forEach(item => {
      ...
      map[item.payer].totalLag += item.billingLag;
    ...
    <div className="gcr-container">
      ...
      <div className="summary-filters">
        <div className="summary-boxes">
          <div className="summary-box" style={{ position: "relative" }}>
            <h4>Average Billing Lag</h4>
            <p>{kpiMetrics.avgBillingLag} Days</p>
      ...
      <div className="charts">
        <div className="chart-card">
          <h3>Monthly Billing Lag (Avg Days)</h3>
          <ResponsiveContainer width="100%" height={200}><LineChart data={filteredMonthlyData} margin={{ top: 5, right: 10, bottom: 0, left: -15 }} ><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="month" /><YAxis /><Tooltip /><Line type="monotone" dataKey="actual" name="Avg Lag" stroke="#8b5cf6" /></LineChart></ResponsiveContainer>
        </div>
        <div className="chart-card">
          <h3>Avg 3 Months vs Last Month</h3>
          <ResponsiveContainer width="100%" height={200}><BarChart data={avgVsLastMonthData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="label" /><YAxis /><Tooltip /><Bar dataKey="lag" fill="#8b5cf6" barSize={40} name="Avg Days" /></BarChart></ResponsiveContainer>
        </div>
        <div className="chart-card">
          <h3>Payer Breakdown (Avg Days)</h3>
          <ResponsiveContainer width="100%" height={200}><BarChart layout="vertical" data={aggregatedPayerData} margin={{ top: 5, right: 10, bottom: 0, left: 40 }}><CartesianGrid strokeDasharray="3 3" /><XAxis type="number" /><YAxis dataKey="name" type="category" width={80} /><Tooltip /><Bar dataKey="avgLag" fill="#8b5cf6" name="Avg Lag" /></BarChart></ResponsiveContainer>
        </div>
      </div>
      <div className="claim-table">
        ...
        <div className="claim-table-body">
          <table>
            <thead><tr>{["Claim ID", "Payer", "Billing Lag (Days)"].map(header => (<th key={header}>{header}</th>))}</tr></thead>
            <tbody>{currentData.map((row) => (<tr key={row.id}><td>{row.id}</td><td>{row.payer}</td><td>{row.billingLag}</td></tr>))}</tbody>
          </table>
        </div>
